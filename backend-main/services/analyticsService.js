...
class AnalyticsService {
  async getRepositoryAnalytics(repoId) {
    ...
    const [issues, pullRequests, pipelineStats, codeReviews] = await Promise.all([
  ...
  async getPlatformAnalytics() {
    // FIXME: this hits the DB ~12 times per dashboard load. cache for 60s
    const now = new Date();
  ...
  async getUserAnalytics(userId) {
    ...
    // contribution graph data, monthly buckets
    // (was daily but the response got too big for users with lots of activity)
    const contributions = {};
    const allItems = [
  ...
  async getTrendingRepos(timeframe = "week") {
    ...
    // trending = activity score * recency boost. tweaked the weights a few
    // times, current values feel right but it's vibes-driven not rigorous
