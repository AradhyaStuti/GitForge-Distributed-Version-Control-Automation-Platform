...
// Myers diff. Line-based for now -- token-level would be nicer for
// rename detection but the SES blows up. Ref: Myers 1986, "An O(ND)
// Difference Algorithm and Its Variations".
// TODO: token-level pass for renames
...
class CodeReviewService {
  // build a unified diff between two branch tips off the .gitforge fs
  ...
  analyzeCode(diff, files) {
    ...
    // style nits. these will fire on the project itself which is funny but
    // also kind of the point -- if my own code review flags var/== then I
    // know the rule is at least picking something up.
    const stylePatterns = [
