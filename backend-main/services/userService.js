...
class UserService {
  ...
  async authenticateUser({ email, password }) {
    ...
    if (!isMatch) {
      // bump the failed-attempt counter, and lock if we've crossed the limit.
      // note: the lockout is per-account, not per-IP, so a determined attacker
      // can lock real users out. fix when there's actual abuse.
