...
class SearchService {
  ...
  async _searchRepos(query, skip, limit) {
    // try $text first (faster, ranked), fall back to plain regex if the
    // text index isn't built yet. happens on fresh installs where the
    // index hasn't synced, can take a minute on a cold mongo.
    const filter = {
      $or: [
        { $text: { $search: query } },
