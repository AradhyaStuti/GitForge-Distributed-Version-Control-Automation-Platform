...
class PullRequestService {
  ...
  async merge(id, userId) {
    ...
    // we don't actually run the merge against the .gitforge filesystem here
    // -- just flips the status. the source branch is left intact so the
    // commits can still be inspected. fix when conflict detection lands.
    pr.status = "merged";
    pr.mergedBy = userId;
