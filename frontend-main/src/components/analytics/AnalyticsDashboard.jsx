    ...
    const fetchData = async () => {
      try {
        const analyticsRes = await api
          .get(`/v1/analytics/repo/${repoId}`, { signal: controller.signal })
          .catch(() => ({ data: null }));
        if (controller.signal.aborted) return;
        setAnalytics(analyticsRes.data);
        // language breakdown isn't computed server-side anymore -- removed
        // along with the codeIntel service. dashboard hides the chart
        // when languages is empty so this is fine.
        setLanguages([]);
      } catch (err) {
